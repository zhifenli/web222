...
function createVideo(src, loop, muted) {
  <video src="https://web222.ca"> </video>;
}
...
function toProvince(postalCode, useLongForm) {
  ...
  try {
    ...
    if (!(/^[A-Z][0-9][A-Z][0-9][A-Z][0-9]$/).test(postalCode)) {
      ...
      if (arr1.includes(postalCode.charAt(0))) {
        if (useLongForm == true) {
          return 'Ontario';
        }
        else {
      ...
      if (arr2.includes(postalCode.charAt(0))) {
        if (useLongForm == true) {
          return 'Quebec';
        }
        else {
...
function normalizeCoord(value) {
  ...
  function generateLicenseLink(licenseCode, includeLicenseAttr) {
    ...
    return <a rel="license" href="http://creativecommons.org/licenses/by-nc/3.0/">Creative Commons Attribution-NonCommercial License</a>
  }
