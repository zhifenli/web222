...
function generateLicenseLink(licenseCode, includeLicenseAttr) {
  ...
  return includeLicenseAttr
    ? `<a href="https://choosealicense.com/no-permission/" rel="license">All Rights Reserved</a>`
...
function buildQuery(query, perPage, page, format) {
  var formats = ['html', 'json', 'xml'];

  if (perPage < 1 || perPage > 100 || page < 1 || !formats.includes(format)) {
    throw new Error();
  }
  return `https://api.web222-example.org/v3/query?query=${encodeURIComponent(
    query
  )}&per_page=${perPage}&page=${page}&format=${format}`;
}
